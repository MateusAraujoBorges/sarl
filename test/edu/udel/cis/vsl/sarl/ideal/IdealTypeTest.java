...
/**
 * The class IdealTypeTest tests methods found in the
 * edu.udel.cis.vsl.sarl.ideal.common package using types
 * 
...
public class IdealTypeTest {
	...
	public void setUp() throws Exception {
		...
		e01 = idealFactory.expression(SymbolicOperator.CAST, real, three); // real
																			// 3
																			// cast
																			// to
																			// integer
		...
		e2 = idealFactory.expression(SymbolicOperator.ADD, integer, five,
				three); // 5 + 3 ADD
		e3 = idealFactory.expression(SymbolicOperator.COND, real, x,
				booleanFactory.trueExpr(), booleanFactory.falseExpr());
		e4 = idealFactory.expression(SymbolicOperator.MULTIPLY, integer, five,
				three); // 5 * 3 MULTIPLY
		e5 = idealFactory.expression(SymbolicOperator.NEGATIVE, integer, five); // -5
																				// NEGATIVE
		e6 = idealFactory.expression(SymbolicOperator.POWER, integer, five,
				three); // 5 ^ 3 POWER
		e7 = idealFactory.expression(SymbolicOperator.SUBTRACT, integer, five,
				three); // 5 - 3 SUBTRACT
		e10 = idealFactory.expression(SymbolicOperator.POWER, integer, five,
				intObj3); // 5 ^ 3 POWER
	...
	@Test
	public void cast() {
		NumericExpression n11 = idealFactory
				.subtract(idealFactory.multiply(intNegOne, x), intTen);
	...
	public void symbolicConstantInequality2() {
		SymbolicConstant x2 = idealFactory
				.symbolicConstant(objectFactory.stringObject("X"), real);
	...
	public void polynomial() {
		...
		SymbolicMap<Monic, Monomial> termMap = idealFactory.emptyMonicMap();
